...
function get_prm($prm) {
  ...
  global $global_dbh,$mod_prm;
  $sql = "SELECT * FROM `".$mod_prm."` WHERE `prm` = '".$prm."'";
  $res = mysql_query($sql, $global_dbh) or die("display_db_query:" . mysql_error());
...
function get_modell_eov_wgs84($eovy, $eovx) {
  ...
  global $global_dbh,$mod_eov_wgs;
  $ey=get_prm("ey");
  $ex=get_prm("ex");
  $sql="SELECT *  FROM `".$mod_eov_wgs."` WHERE `y` > ".$eovy."-".$ey." AND `y` < ".$eovy."+".$ey." AND `x` > ".$eovx."-".$ex." AND `x` < ".$eovx."+".$ex."";
  $res = mysql_query($sql, $global_dbh) or die("display_db_query:" . mysql_error());
 
  $count = mysql_num_fields($res) or die("display_db_query:" . mysql_error());
